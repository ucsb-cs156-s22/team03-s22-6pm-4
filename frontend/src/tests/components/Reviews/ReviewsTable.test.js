...
describe("ReviewsTable tests", () => {
  ...
  test("Has the expected column headers and content for adminUser", () => {
    ...
    const { getByText, getByTestId } = render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <ReviewsTable reviews={reviewsFixtures.threeMenuReviews} currentUser={currentUser} />
        </MemoryRouter>
      </QueryClientProvider>

    );

    const expectedHeaders = ['ID', 'Item ID', 'Reviewer Email', 'Stars (Rating)', 'Date Reviewed', 'Comments'];
